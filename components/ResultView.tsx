...
import { Cpu, ShieldCheck, TrendingUp, RefreshCw } from 'lucide-react';
...
const ResultView: React.FC<ResultViewProps> = ({ result, onReset }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col relative overflow-y-auto no-scrollbar">
            {/* ===== Recommendation Card ===== */}
            <div className="mt-6 mx-6 relative group">
                {/* Glow */}
            ...
            <div className="flex-1 flex flex-col justify-end pb-8 px-4 gap-4 mt-4">
                {/* Safe Tiles */}
                <div className="glass-panel p-3 rounded-xl flex-1 border border-blue-500/30 bg-blue-900/20 max-h-fit">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {safeTiles && safeTiles.length > 0 ? (
                            safeTiles.map((tile, idx) => (
                                <MahjongTile key={`safe-${idx}`} code={tile} />
                            ))
                        ) : (
